...
def test_compute_spectrogram():
    ...
    expected_frames = int((sample_rate * duration - config.window_size) / config.hop_width + 1)
    assert spec.shape == (expected_frames, config.num_mel_bins)
    
    # Check values
    assert tf.reduce_all(tf.math.is_finite(spec))
    assert tf.reduce_all(spec >= np.log(config.clip_min_value))  # Log-mel spectrogram should be >= log(clip_min_value)
