...
class RunLengthEncodingTest(tf.test.TestCase):
    ...
    def test_encode_and_index_note_sequence(self):
        ...
        ns.notes.add(start_time=1.0, end_time=1.1, pitch=61, velocity=100)
        ns.notes.add(start_time=2.0, end_time=2.1, pitch=62, velocity=100)
        ns.notes.add(start_time=3.0, end_time=3.1, pitch=63, velocity=100)
        ns.total_time = ns.notes[-1].end_time

        frame_times = np.arange(0, 4, step=0.001)

        event_times, event_values = note_sequences.note_sequence_to_onsets(ns)
        events, event_start_indices, event_end_indices, _, _ = (
        ...
        expected_events = (
            [1] * 100 + [162] + [1] * 100 + [163] + [1] * 100 + [164] + [1] * 100
        )
    ...
    def test_encode_and_index_note_sequence_velocity(self):
        ns = note_seq.NoteSequence()
        ns.notes.add(start_time=1.0, end_time=3.0, pitch=61, velocity=1)
        ns.notes.add(start_time=2.0, end_time=4.0, pitch=62, velocity=127)
        ns.total_time = ns.notes[-1].end_time

        frame_times = np.arange(0, 4, step=0.001)

        event_times, event_values = note_sequences.note_sequence_to_onsets_and_offsets(
            ns
        )
        events, event_start_indices, event_end_indices, _, _ = (
            run_length_encoding.encode_and_index_events(
    ...
    def test_encode_and_index_note_sequence_multitrack(self):
        ns = note_seq.NoteSequence()
        ns.notes.add(start_time=0.0, end_time=1.0, pitch=37, velocity=127, is_drum=True)
        ns.notes.add(start_time=1.0, end_time=3.0, pitch=61, velocity=127, program=0)
        ns.notes.add(start_time=2.0, end_time=4.0, pitch=62, velocity=127, program=40)
        ns.total_time = ns.notes[-1].end_time
        ...
        expected_state_tokens = [codec.encode_event(e) for e in expected_state_events]
    ...
    def test_encode_and_index_note_sequence_last_token_alignment(self):
        ns = note_seq.NoteSequence()
        ns.notes.add(start_time=0.0, end_time=0.1, pitch=60, velocity=100)
        ns.total_time = ns.notes[-1].end_time

        frame_times = np.arange(0, 1.008, step=0.008)
    ...
    def test_decode_note_sequence_events(self):
        ...
        expected_ns.notes.add(pitch=60, velocity=100, start_time=0.25, end_time=0.26)
        expected_ns.notes.add(pitch=61, velocity=100, start_time=0.50, end_time=0.51)
    ...
    def test_decode_note_sequence_events_onsets_only(self):
        ...
        self.assertEqual(0, dropped_events)
        expected_ns = note_seq.NoteSequence(ticks_per_quarter=220)
        expected_ns.notes.add(pitch=60, velocity=100, start_time=0.05, end_time=0.06)
        expected_ns.notes.add(pitch=61, velocity=100, start_time=0.25, end_time=0.26)
    ...
    def test_decode_note_sequence_events_velocity(self):
        ...
        self.assertEqual(0, dropped_events)
        expected_ns = note_seq.NoteSequence(ticks_per_quarter=220)
        expected_ns.notes.add(pitch=60, velocity=127, start_time=0.05, end_time=0.25)
    ...
    def test_decode_note_sequence_events_missing_offset(self):
        ...
        self.assertEqual(0, dropped_events)
        expected_ns = note_seq.NoteSequence(ticks_per_quarter=220)
        expected_ns.notes.add(pitch=60, velocity=127, start_time=0.05, end_time=0.10)
        expected_ns.notes.add(pitch=60, velocity=127, start_time=0.10, end_time=0.25)
        expected_ns.total_time = 0.25
        self.assertProtoEquals(expected_ns, ns)
    ...
    def test_decode_note_sequence_events_multitrack(self):
        ...
        expected_ns.notes.add(
            pitch=60, velocity=127, start_time=0.05, end_time=0.25, program=40
        )
        expected_ns.total_time = 0.25
        self.assertProtoEquals(expected_ns, ns)
    ...
    def test_decode_note_sequence_events_allow_event_at_exactly_max_time(self):
        ...
        self.assertEqual(0, dropped_events)
        expected_ns = note_seq.NoteSequence(ticks_per_quarter=220)
        expected_ns.notes.add(pitch=60, velocity=100, start_time=1.00, end_time=1.01)
        expected_ns.notes.add(pitch=61, velocity=100, start_time=1.25, end_time=1.26)
    ...
    def test_decode_note_sequence_events_dropped_events(self):
        ...
        expected_ns = note_seq.NoteSequence(ticks_per_quarter=220)
        expected_ns.notes.add(pitch=60, velocity=100, start_time=1.05, end_time=1.06)
    ...
    def test_decode_note_sequence_events_invalid_events(self):
        ...
        self.assertEqual(0, dropped_events)
        expected_ns = note_seq.NoteSequence(ticks_per_quarter=220)
        expected_ns.notes.add(pitch=60, velocity=100, start_time=0.50, end_time=0.51)
        expected_ns.total_time = 0.51
        self.assertProtoEquals(expected_ns, ns)
