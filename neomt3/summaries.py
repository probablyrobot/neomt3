...
from typing import Any, Mapping, Optional, Sequence, Tuple, Callable, Dict
...
class Image(MetricValue):
    """Image metric value."""
    def __init__(self, image, max_outputs=None):
        self.image = image
        self.max_outputs = max_outputs

class Audio(MetricValue):
    """Audio metric value."""
    def __init__(self, audiodata, sample_rate, max_outputs=None):
...
def prettymidi_pianoroll(
  ...
  for inst_name, all_prs in track_pianorolls.items():
    ...
    summaries[f'{inst_name} pretty_midi pianoroll'] = Image(
...
def transcription_summaries(
  ...
  return {
      'input_with_transcription': Audio(
...
def compute_metrics(
    ...
    if target_sequence_length is not None:
        mask = tf.sequence_mask(target_sequence_length)
        masked_accuracy = tf.reduce_mean(
            tf.cast(tf.equal(predictions, targets), tf.float32) * 
            tf.cast(mask, tf.float32)
        )
        metrics['masked_accuracy'] = masked_accuracy
...
def visualize_pianoroll(
    ...
    plt.imshow(pianoroll.T, aspect='auto', origin='lower')
    plt.colorbar()
    plt.title(title)
