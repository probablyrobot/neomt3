...
from neomt3 import event_codec, note_sequences, run_length_encoding, vocabularies

S = TypeVar("S")
T = TypeVar("T")

CombineExamplesFunctionType = Callable[[Sequence[Mapping[str, Any]]], Mapping[str, Any]]
...
def combine_predictions_by_id(
    ...
    return {
        id: combine_predictions_fn(preds) for id, preds in predictions_by_id.items()
    }
...
def decode_and_combine_predictions(
    ...
    decode_tokens_fn: Callable[[S, Sequence[int], int, Optional[int]], Tuple[int, int]],
...
def frame_metrics(
    ref_pianoroll: np.ndarray, est_pianoroll: np.ndarray, velocity_threshold: int
    ...
    precision, recall, f1, _ = sklearn.metrics.precision_recall_fscore_support(
        ref_frames_bool.flatten(), est_frames_bool.flatten(), labels=[True, False]
    )
